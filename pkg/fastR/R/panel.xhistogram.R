...
function (x, dcol = trellis.par.get("plot.line")$col, dlwd = 2, 
    gcol = trellis.par.get("add.line")$col, glwd = 2, dmath = dnorm, 
    dn = 100, args = NULL, labels = FALSE, density = FALSE, fit = NULL, 
    start = NULL, type = "density", v, h,  ...) 
{
    panel.histogram(x, type = type, ...)
    if (labels) {
        hist.data <- hist(x, plot = FALSE, ...)
        if (type == "count") {
            aa <- max(hist.data$counts) * 0.01
            grid.text(label = as.character(round(hist.data$counts, 3)), 
                x = hist.data$mids, y = aa + hist.data$counts, just = c("centre", 
                  "bottom"), default.units = "native")
        }
        else if (type == "percent") {
            ss <- sum(hist.data$counts)
            aa <- max(0.01 * hist.data$counts/ss)
            cat(hist.data$counts)
            cat("\n")
            cat(hist.data$counts/ss)
            cat("\n")
            cat(ss)
            cat("\n")
            print(hist.data)
            cat("\n")
            grid.text(label = as.character(round(hist.data$counts/ss, 
                3)), x = hist.data$mids, y = aa + (hist.data$counts/ss), just = c("centre", 
                "bottom"), default.units = "native")
        }
        else {
            aa <- max(hist.data$density) * 0.01
            grid.text(label = as.character(round(hist.data$density, 3)), 
                x = hist.data$mids, y = aa + hist.data$density, just = c("centre", 
                  "bottom"), default.units = "native")
        }
    }
    if (!is.null(fit)) {
